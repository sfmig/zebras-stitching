...
for i in tqdm(range(1, video_data.shape[0])):
    ...
    for j in range(num_animals):
        ...
        if np.any(min_y[fixed_index, j] < 0) or np.any(min_x[fixed_index, j]  < 0) or np.any(max_y[fixed_index, j]  < 0) or np.any(max_x[fixed_index, j]  < 0):
            continue

        if np.any(min_y[moving_index, j] < 0) or np.any(min_x[moving_index, j]  < 0) or np.any(max_y[moving_index, j]  < 0) or np.any(max_x[moving_index, j]  < 0):
            continue

        # Mask out the area around the animal
        fixed_mask[min_y[fixed_index, j] - buffer:max_y[fixed_index, j] + buffer,
        min_x[fixed_index, j] - buffer:max_x[fixed_index, j] + buffer] = 0
        moving_mask[min_y[moving_index, j] - buffer:max_y[moving_index, j] + buffer,
        min_x[moving_index, j] - buffer:max_x[moving_index, j] + buffer] = 0
...
transform_df = pd.concat([pd.DataFrame({
    'theta': [0],
    'tx': [0],
    'ty': [0]
}), transform_df], ignore_index=True)
transform_df.head()
