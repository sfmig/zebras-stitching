...
mesh_path = Path("/workspace/datasets/project/odm_meshing/odm_25dmesh.ply")
...
)  # time, individuals, kpts, space (homogeneous)
...
for frame_filename in list_frame_filenames:
    ...
    pt2D_pixels_homogeneous = position_homogeneous.sel(time=frame_idx).values.reshape(
        -1, 3
    )
    ...
    bearings_rotated_to_wcs = (
        pose.get_R_cam_to_world() @ pt2D_bearings_homogeneous_ccs.T
    ).T
...
csv_path = Path("data") / f"{Path(points_2d_slp).stem}_sfm_transforms_{timestamp}.csv"
